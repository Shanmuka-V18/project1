...
export async function GET(request: Request) {
  try {
    ...
    page.drawText(formatPdfCurrency(result.currentMonth.revenue), { x: 200, y, size: 10, font: fontBold, color: rgb(0.02, 0.6, 0.4) });

    page.drawText('Total Operating Expenses:', { x: 320, y, size: 10, font: fontBold, color: darkColor });
    page.drawText(formatPdfCurrency(result.currentMonth.expenses), { x: 460, y, size: 10, font: fontBold, color: rgb(0.85, 0.15, 0.15) });

    y -= 25;
    page.drawText('Net Operating Income (EBITDA):', { x: 55, y, size: 11, font: fontBold, color: primaryColor });
    page.drawText(formatPdfCurrency(result.currentMonth.netIncome), { x: 230, y, size: 11, font: fontBold, color: primaryColor });
    ...
    if (incomeCats.length === 0) {
      ...
      incomeCats.forEach(([cat, val]) => {
        ...
        page.drawText(formatPdfCurrency(val), { x: 480, y, size: 10, font, color: darkColor });
        y -= 18;
      });
    }
    ...
    console.error('PnL PDF Generation Error:', err);
